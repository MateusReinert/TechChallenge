export const TRANSACTION_ATTACHMENT_ACCEPT = ".pdf,.png,.jpg,.jpeg";

export const TRANSACTION_ATTACHMENT_MAX_SIZE = 5 * 1024 * 1024;

export const TRANSACTION_ATTACHMENT_MAX_SIZE_LABEL = "5 MB";
...
function getBase64Data(base64: string) {
  const match = base64.match(/^data:([^;,]+);base64,([A-Za-z0-9+/]*={0,2})$/);
...
export function validateTransactionAttachment(
  ...
  if (attachment.size > TRANSACTION_ATTACHMENT_MAX_SIZE) {
  ...
  const base64Data = getBase64Data(attachment.base64);

  if (!base64Data) {
    return `O conteúdo do arquivo ${attachment.name} é inválido.`;
  }

  if (base64Data.mimeType !== normalizedMimeType) {
    return `O conteúdo do arquivo ${attachment.name} não corresponde ao tipo informado.`;
  }

  const decodedSize = getDecodedBase64Size(base64Data.content);

  if (decodedSize <= 0 || decodedSize > TRANSACTION_ATTACHMENT_MAX_SIZE) {
...
export function validateTransactionAttachments(
  ...
  for (const attachment of attachments) {
    const validationError = validateTransactionAttachment(attachment);
